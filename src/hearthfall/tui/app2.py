...
from textual.widgets import RichLog, Static, Footer, Label, Button
from textual.command import Provider, Hit, Hits
from textual.binding import Binding
from rich.text import Text

from hearthfall import VERSION
from hearthfall.engine import turn, balance
from hearthfall.engine.rng import Rng
from hearthfall.engine.state import GameState, Season, Outcome
...
class ActionProvider(Provider):
    async def search(self, query: str) -> Hits:
        ...
        for title, action in actions:
            ...
            if score > 0:
                yield Hit(score, matcher.highlight(title), lambda a=action: self.app.action_dispatch(a))
...
class HearthfallApp(App):
    ...
    def run_season(self) -> None:
        from hearthfall.engine.turn import run_until_interrupted, InterruptReason
        ...
        for entry in self.state.chronicle:
            log.write(f"[bold]{entry.season.value.title()}, Year {entry.turn // 4}[/bold]")
