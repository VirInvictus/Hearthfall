...
> **Status: v0.23.2. SP 7 (composition) is complete: the militia is an assembly, the counter-web prices the mix, and a band's read goes stale behind its border: a camp that has quietly reinforced is caught only by walking out to look. The granary trade is now a reading trade.** Playable start to finish, and
